...
void init_circular_buffer(cb_ptr buffer) {
...
bool is_circular_buffer_empty(cb_ptr buffer) {
	return buffer->_size == 0;
}

bool is_circular_buffer_full(cb_ptr buffer) {
	return buffer->_size == MAX_BUFFER_SIZE;
}

void push_circular_buffer(cb_ptr buffer, struct packet_info pi) {
...
void pop_circular_buffer(cb_ptr buffer) {
...
int get_circular_buffer_size(cb_ptr buffer) {
	return buffer->_size;
}

struct packet_info get_circular_buffer_front(cb_ptr buffer) {
	return buffer->_data[buffer->_start];
}

int get_circular_buffer_isp_count(cb_ptr buffer, int isp_id) {
