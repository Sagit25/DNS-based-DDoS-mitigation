...
long do_get_threshold(u32 l_ip) {
	struct puzzle_policy *policy;
	if(unlikely(!find_puzzle_policy(l_ip, &policy)))
		return 0;
...
u32 do_set_threshold(u32 l_ip, u32 threshold) {
	struct puzzle_policy *policy;
	if(likely(find_puzzle_policy(l_ip, &policy)))
...
void print_dns_info() {
    printf("local_dns_ip: %d\n", puzzle_dns.ip);
    printf("local_dns_port: %d\n", puzzle_dns.port);
    return;
}
...
long do_set_local_dns(u32 ip, u32 port, u32 threshold) {
    puzzle_dns.ip = ip;
